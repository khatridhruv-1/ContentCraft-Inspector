...
export default function Welcome() {
  ...
  return (
    <>
      ...
      {/* Fixed top bar with product name + theme toggle */}
      <header
        data-testid="welcome-theme-toggle"
        className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-3 bg-background/80 backdrop-blur-sm border-b border-border"
      >
        <span className="font-bold text-foreground text-lg">ContentCraft-Inspector</span>
        <ModeToggle />
      </header>

      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 flex flex-col items-center pt-24 pb-16 px-6 relative">
        {/* Blobs */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-20 left-20 w-72 h-72 bg-blue-200 dark:bg-blue-900 rounded-full mix-blend-multiply dark:mix-blend-normal filter blur-xl opacity-60 dark:opacity-[0.15] animate-blob" />
          <div className="absolute top-40 right-20 w-64 h-64 bg-purple-200 dark:bg-purple-900 rounded-full mix-blend-multiply dark:mix-blend-normal filter blur-xl opacity-60 dark:opacity-[0.15] animate-blob animation-delay-2000" />
          <div className="absolute bottom-20 left-1/2 w-80 h-80 bg-pink-200 dark:bg-pink-900 rounded-full mix-blend-multiply dark:mix-blend-normal filter blur-xl opacity-50 dark:opacity-[0.10] animate-blob animation-delay-4000" />
          <div className="absolute top-1/3 left-1/4 w-48 h-48 bg-indigo-200 dark:bg-indigo-900 rounded-full mix-blend-multiply dark:mix-blend-normal filter blur-xl opacity-40 dark:opacity-[0.10] animate-blob animation-delay-3000" />
          <div className="absolute bottom-1/3 right-1/4 w-56 h-56 bg-cyan-200 dark:bg-cyan-900 rounded-full mix-blend-multiply dark:mix-blend-normal filter blur-xl opacity-40 dark:opacity-[0.10] animate-blob animation-delay-5000" />
        </div>
        ...
        <div className="relative z-10 max-w-4xl w-full">
          ...
          >
            <motion.div variants={heroItemVariants} className="flex justify-center mb-6">
              <motion.div
                className="bg-card p-4 rounded-2xl shadow-xl"
            ...
            <motion.h1 variants={heroItemVariants} className="text-4xl md:text-6xl font-bold text-foreground mb-6">
              Welcome to{' '}
          ...
          >
            {features.map(({ icon: Icon, title, description, bg, iconColor }) => (
              <motion.div
                ...
                className="bg-card text-card-foreground rounded-2xl p-6 shadow-lg hover:shadow-xl border border-transparent hover:border-indigo-100 dark:hover:border-indigo-800 transition-all duration-200 cursor-default"
              >
                <motion.div
          ...
          >
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              className="bg-white dark:bg-slate-800 text-gray-800 dark:text-gray-100 px-8 py-4 rounded-xl text-lg font-semibold shadow-lg hover:shadow-xl border border-gray-200 dark:border-slate-600 transition-shadow"
            >
              Sign In
            </motion.button>
          </motion.div>
